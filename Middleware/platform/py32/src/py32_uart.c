...
void UART_Init(void)
{
	...
	HAL_UART_Receive_IT(&UartHandle, g_cmdRecv.buf[g_cmdRecv.bufRecv], CMD_RECV_BUF_SIZE);
}
...
void UartReceive_IDLE(void)  
{  		
	HAL_UART_IRQHandler(&UartHandle);

	g_cmdRecv.idleCnt = 0;
	g_cmdRecv.curIndex = CMD_RECV_BUF_SIZE - UartHandle.RxXferCount;
...
void UART_DirectSend(uint8_t* buf, uint16_t len)
{
    ...
    HAL_UART_Transmit(&UartHandle,buf,len,10);
}
#if 1
